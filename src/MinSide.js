  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const auctionResponse = await axios.get('http://localhost:8082/api/myauctions', {
          headers: {
        ...
        const messageResponse = await axios.get('http://localhost:8082/api/mymessages', {
          headers: {
            'Authorization': `Bearer ${token}` // Include the token in the headers
          }
        });
        ...
        const userResponse = await axios.get('http://localhost:8082/api/userdetails', {
          headers: {
            'Authorization': `Bearer ${token}` // Include the token in the headers
          }
        });
